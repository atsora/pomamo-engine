...
namespace Lemoine.Alert
{
  ...
  /// </summary>
  [Serializable]
  public class GroupedConfigEMailAction : GenericEMailAction
  {
    #region Members
    ...
    static readonly ILog log = LogManager.GetLogger (typeof (GroupedConfigEMailAction).FullName);
    ...
    /// </summary>
    public GroupedConfigEMailAction ()
    {
    }
    ...
    string GetFromXslDefinition (XslDefinition xslDefinition, XmlElement data)
    {
      ...
      if (log.IsDebugEnabled) {
        log.Debug ($"GetFromXslDefinition:");
      }
    ...
    {
      if (log.IsDebugEnabled) {
        log.Debug ($"GetMatchedEmailConfigs:");
      }
      ...
      using (IDAOSession session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        ...
        foreach (IEmailConfig emailConfig in activeEmailConfigs) {
          if (log.IsDebugEnabled) {
            log.Debug ($"GetMatchedEmailConfigs: emailconfig {emailConfig.Name}");
          }
    ...
    {
      if (log.IsDebugEnabled) {
        log.Debug ($"GetDateTime: m_dateTime={m_dateTime.Xslt}");
      }
      string dateTimeString = GetFromXslDefinition (m_dateTime, data);
      if (log.IsDebugEnabled) {
        log.Debug ($"GetDateTime: dateTimeString={dateTimeString} ");
      }
      DateTime dateTime = DateTime.UtcNow;
      if (null == dateTimeString) {
        if (log.IsDebugEnabled) {
          log.DebugFormat ("GetDateTime: " +
