...
namespace Lemoine.Plugin.AutoReasonPalletChangeSequence
{
  /// <summary>
  /// Detect sequences of type "Pallet change"
  /// </summary>
    ...
    , Lemoine.Extensions.AutoReason.IAutoReasonExtension
  {
    ...
    public AutoReasonExtension () : base (Plugin.DEFAULT_REASON_TRANSLATION_KEY, "AutoReason.PalletChangeSequence")
    {
    }
    ...
    protected override void Check ()
    {
      using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        using (var transaction = session.BeginReadOnlyTransaction ("AutoReason.PalletChangeSequence.Check")) {
            ...
            if (nextDateTime < this.DateTime) // Should not append
{
            ...
            // * the sequence kind is "PalletChange"
            // * start date time is between DateTime and nextDateTime (including both limits of the range)
            foreach (var nextSlot in nextSlots) {
              SetActive ();
              // Sequence is stop?
              bool isStop = (nextSlot.Sequence != null && nextSlot.Sequence.Kind == SequenceKind.AutoPalletChange);
