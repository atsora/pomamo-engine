...
namespace Lemoine.Plugin.ReasonToProductionState
{
    ...
    , Pulse.Extensions.Configuration.IConfigurationWithMachineFilter
  {
    ...
    /// <summary>
    /// Reason that is the source of the production state
    /// </summary>
    ...
    public override bool IsValid (out IEnumerable<string> errors)
    {
      ...
      using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        using (var transaction = session.BeginReadOnlyDeferrableTransaction ("Plugin.RunningMachineModeIsProduction.IsValidConfiguration")) {
          ...
          if (this.ReasonId <= 0) {
            string message = $"invalid reason id {this.ReasonId}: not strictly positive";
            log.Error ($"IsValid: {message}");
            errorList.Add (message);
          }
          else {
            ...
            if (reason is null) {
              string message = $"invalid reason id {this.ReasonId}: unknown reason";
              log.Error ($"IsValid: {message}");
              errorList.Add (message);
            }
          }
