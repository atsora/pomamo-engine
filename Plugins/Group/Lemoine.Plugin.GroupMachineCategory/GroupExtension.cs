      ...
      get
      {
        if (null == m_groups) { // Not initialized yet
          using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
            ...
            var groups = machines
              .Where (m => null != m.Category)
              .GroupBy (m => m.Category.Id, (i, ms) => new GroupFromMachineList (BuildId (i), ms.First ().Category.Name, "MachineCategory", ms.OrderBy (a => a.DisplayPriority), ms.First ().Category.DisplayPriority, true, m_configuration.ZoomInMachineSelection));
            m_groups = groups
              .OrderBy (g => g.Name)
