      ...
      get
      {
        if (null == m_groups) { // Not initialized yet
          using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
            ...
            var groups = machines
              .Where (m => null != m.Department)
              .GroupBy (m => m.Department.Id, (i, ms) => new GroupFromMachineList (BuildId (i), ms.First ().Department.Name, "Department", ms.OrderBy (a => a.DisplayPriority), ms.First ().Department.DisplayPriority, true, m_configuration.ZoomInMachineSelection));
            m_groups = groups
              .OrderBy (g => g.Name)
