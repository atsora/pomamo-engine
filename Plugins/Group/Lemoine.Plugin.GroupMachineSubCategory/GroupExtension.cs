      ...
      get
      {
        if (null == m_groups) { // Not initialized yet
          using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
            ...
            var groups = machines
              .Where (m => null != m.SubCategory)
              .GroupBy (m => m.SubCategory.Id, (i, ms) => new GroupFromMachineList (BuildId (i), ms.First ().SubCategory.Name, "MachineSubCategory", ms.OrderBy (a => a.DisplayPriority), ms.First ().SubCategory.DisplayPriority, true, m_configuration.ZoomInMachineSelection));
            m_groups = groups
              .OrderBy (g => g.Name)
