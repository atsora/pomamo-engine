...
namespace Lemoine.Plugin.AlertConfigFile
{
  ...
  internal sealed class AlertConfig
  {
    ...
    {
      string pathRootedDirectoryPath = new AlertConfigDirectory (directoryPath).AbsolutePath;
      try {
    ...
    {
      try {
        var alertConfigDirectory = new AlertConfigDirectory (directory);
        return Path.Combine (alertConfigDirectory.AbsolutePath, fileName);
      }
      catch (Exception ex) {
        log.Error ($"GetFilePath: getting the file path for {directory}/{fileName} failed", ex);
        throw;
      }
    }
    ...
    public void AddConfigFile (string url)
    {
