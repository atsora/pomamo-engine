...
namespace Lemoine.Plugin.ProductionTargetPerMachine
{
    ...
    : Lemoine.Extensions.Configuration.IConfiguration
  {
    ...
    [DefaultValue (0.0)]
    public double TargetPerHour { get; set; } = 0.0;
    ...
    public bool IsValid (out IEnumerable<string> errors)
    {
      ...
      if (0 != this.MachineFilterId) {
        ...
        using (var transaction = session.BeginReadOnlyTransaction ("Lemoine.Plugin.ProductionTargetPerMachine.Configuration.MachineFilter")) {
      ...
      if (!(this.MachineIds is null) && this.MachineIds.Any ()) {
        using (var session = ModelDAOHelper.DAOFactory.OpenSession ())
        using (var transaction = session.BeginReadOnlyTransaction ("Lemoine.Plugin.ProductionTargetPerMachine.Configuration.Machine")) {
