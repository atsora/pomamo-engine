...
{
  public class ManualReasonEnd
    : Lemoine.Extensions.NotConfigurableExtension
    , IDynamicTimeExtension
  {
    ILog log = LogManager.GetLogger (typeof (NextMachineMode).FullName);

    static readonly string TIMEOUT_KEY = "SameMachineMode.ManualReasonEnd.Timeout";
    ...
    public string Name => "ManualReasonEnd";
    ...
    public DynamicTimeApplicableStatus IsApplicableAt (DateTime dateTime)
    {
      using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        using (var transaction = session.BeginReadOnlyTransaction ("SameMachineMode.ManualReasonEnd.IsApplicableAt")) {
          var reasonSlotAt = ModelDAOHelper.DAOFactory.ReasonSlotDAO
            .FindAt (this.Machine, dateTime);
          if (null != reasonSlotAt) {
            return DynamicTimeApplicableStatus.YesAtDateTime;
          }
          else {
            if (IsFactAfter (dateTime)) { // TODO: ... ?
    ...
    public IDynamicTimeResponse Get (DateTime dateTime, UtcDateTimeRange hint, UtcDateTimeRange limit)
    {
      ...
      using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        using (var transaction = session.BeginReadOnlyTransaction ("DynamicTime.ManualReasonEnd", TransactionLevel.ReadCommitted, transactionLevelOptional: true)) {
          int machineModeId = 0;
