...
{
  /// <summary>
  /// Dynamic time: NextCycleStart
  /// 
  /// Start date/time of the next cycle.
  /// There must be no active cycle at DateTime
  /// </summary>
  public class NextCycleStart
    : Lemoine.Extensions.NotConfigurableExtension
    , IDynamicTimeExtension
  {
    static readonly string APPLICABLE_TIME_SPAN_KEY =
      "DynamicTimesCycle.NextCycleStart.ApplicableTimeSpan";
    static readonly TimeSpan APPLICABLE_TIME_SPAN_DEFAULT = TimeSpan.FromDays (7);

    ILog log = LogManager.GetLogger (typeof (NextCycleStart).FullName);

    public IMachine Machine { get; private set; }

    public string Name => "NextCycleStart";
        ...
        using (var transaction = session.BeginReadOnlyTransaction ("DynamicTimesCycle.PreviousCycleEnd")) {
          ...
          if (null != cycleAt) {
            ...
            if (null != nextCycle) {
              ...
              else { // No begin date/time for the next cycle
                log.Error ($"Get: nextCycle at {nextCycle.DateTime} but with no begin date/time");
                return this.CreateFinal (nextCycle.DateTime);
              }
            }
            else { // No next cycle
    ...
    public bool Initialize (IMachine machine, string parameter)
    {
      ...
      log = LogManager.GetLogger (typeof (NextCycleStart).FullName + "." + machine.Id);
