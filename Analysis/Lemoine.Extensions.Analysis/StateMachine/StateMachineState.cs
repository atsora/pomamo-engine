...
{
  /// <summary>
  /// StateMachineState: state that is made of a sub state machine
  /// 
  /// Not implemented yet
  /// </summary>
    ...
    where TInternalContext: IContext<TInternalContext>
  {
    ...
    readonly IState<TInternalContext> m_internalStateMachine;
    ...
    public StateMachineState (string name, State<TInternalContext> internalStateMachine, IState<TContext> nextState, IState<TContext> exceptionState, IState<TContext> maxTimeState = null)
    ...
    /// <param name="exceptionState"></param>
    /// <param name="maxTimeState">if not null, test the maximum time after running the internal state machine</param>
    public StateMachineState (string name, string perfName, State<TInternalContext> internalStateMachine, IState<TContext> nextState, IState<TContext> exceptionState, IState<TContext> maxTimeState = null)
      : base (exceptionState)
    {
      m_name = name;
      m_perfName = perfName;
...
//      var stateMachine = new StateMachine<TInternalContext> (this.Context, m_internalStateMachine);
/*
      var result = m_action (this.Context);
      if (!result) {
