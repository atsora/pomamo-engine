...
namespace Lemoine.Stamping.StampingEventHandlers
{
  ...
  /// </summary>
  public class CommentHandler : IStampingEventHandler
  {
    readonly ILog log = LogManager.GetLogger (typeof (CommentHandler).FullName);

    readonly StampingData m_stampingData;
    readonly ICommentParser m_commentParser;

    /// <summary>
    /// Constructor
    /// </summary>
    public CommentHandler (IStamper stamper, StampingData stampingData, ICommentParser commentParser)
    {
      this.Stamper = stamper;
      m_stampingData = stampingData;
      m_commentParser = commentParser;
    }
    ...
    {
      var parsingSuccess = m_commentParser.ParseComment (message, m_stampingData);
      if (log.IsDebugEnabled) {
        log.Debug ($"SetComment: {message} => success={parsingSuccess}");
      }
