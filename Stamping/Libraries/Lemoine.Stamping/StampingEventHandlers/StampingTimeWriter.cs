...
{
  /// <summary>
  /// Add a comment at the end of the program with the total stamping time
  /// </summary>
  public class StampingTimeWriter
    : IStampingEventHandler
  {
    readonly ILog log = LogManager.GetLogger (typeof (StampingTimeWriter).FullName);

    readonly DateTime m_startDateTime = DateTime.UtcNow;

    /// <summary>
    /// Constructor
    /// </summary>
    public StampingTimeWriter (IStamper stamper)
    {
    ...
    {
      if (edit && (0 == level)) {
        var duration = DateTime.UtcNow.Subtract (m_startDateTime);
        var line = $"(StampingTime={duration})";
