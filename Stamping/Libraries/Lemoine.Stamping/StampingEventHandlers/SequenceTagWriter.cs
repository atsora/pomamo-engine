...
{
  /// <summary>
  /// Add a comment when a new sequence is detected
  /// </summary>
  public class SequenceTagWriter
    : IStampingEventHandler
  {
    readonly ILog log = LogManager.GetLogger (typeof (SequenceTagWriter).FullName);

    /// <summary>
    /// Constructor
    /// </summary>
    public SequenceTagWriter (IStamper stamper)
    {
    ...
    {
      var line = sequenceKind switch {
        SequenceKind.AutoPalletChange => $"(Pallet change sequence)",
        SequenceKind.Machining => $"(SEQ)",
        SequenceKind.NonMachining => $"(NMSEQ)",
        SequenceKind.OptionalStop => $"(OPTIONAL STOP)",
        SequenceKind.Stop => $"(STOP)",
