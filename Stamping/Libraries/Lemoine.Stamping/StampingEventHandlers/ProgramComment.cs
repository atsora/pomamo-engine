...
{
  /// <summary>
  /// Get the ProgramComment as the first comment in the first blocks
  /// 
  /// This is not required for standard g-codes
  /// </summary>
  public class ProgramComment
    : IStampingEventHandler
  {
    readonly ILog log = LogManager.GetLogger (typeof (ProgramComment).FullName);
    ...
    /// Constructor
    /// </summary>
    public ProgramComment (IStamper stamper, StampingData stampingData)
    {
    ...
    {
      if ((MAX_LINES == m_line) && !m_completed) {
        log.Error ($"NotifyNewBlock: no comment was set in the header");
      }
      m_line++;
    ...
    {
      if (!m_completed && (m_line < MAX_LINES)) {
        m_stampingData.Add ("ProgramComment", message.Trim ());
        m_completed = true;
      }
