...
namespace Lemoine.Stamping.StampingEventHandlers
{
  ...
  {
    readonly ILog log = LogManager.GetLogger (typeof (DelayMachiningSequence).FullName);

    bool m_pendingMachiningSequence = false;

    /// <summary>
    /// Constructor
    /// </summary>
    public DelayMachiningSequence (IStamper stamper)
    {
    ...
    {
      if (sequenceKind == SequenceKind.Machining) {
        m_pendingMachiningSequence = true;
      }
      else {
    ...
    {
      if (m_pendingMachiningSequence) {
        this.Next?.StartSequence (Model.SequenceKind.Machining);
        m_pendingMachiningSequence = false;
      }
    }
