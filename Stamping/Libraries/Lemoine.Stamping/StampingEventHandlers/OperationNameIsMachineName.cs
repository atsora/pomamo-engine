...
{
  /// <summary>
  /// Push OperationName = MachineName
  /// </summary>
  public class OperationNameIsMachineName
    : IStampingEventHandler
  {
    readonly ILog log = LogManager.GetLogger (typeof (OperationNameIsMachineName).FullName);
    ...
    /// Constructor
    /// </summary>
    public OperationNameIsMachineName (IStamper stamper, StampingData stampingData)
    {
      this.Stamper = stamper;
      m_stampingData = stampingData;

      if (stampingData.TryGet ("MachineName", out string? machineName)) {
        if (log.IsDebugEnabled) {
          log.Debug ($"OperationNameIsMachineName: OperationName={machineName}");
        }
        stampingData.Add ("OperationName", machineName);
      }
    }
    ...
    public void SetData (string key, object v)
    {
      if (key.Equals ("MachineName")) {
        this.Next?.SetData ("OperationName", v);
      }
