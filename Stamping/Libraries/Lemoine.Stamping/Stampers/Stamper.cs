...
{
  /// <summary>
  /// Default stamper: read a NC Program and write a new NC Program with stamp information
  /// </summary>
    ...
    , IAsyncDisposable
  {
    ...
    /// <see cref="IStamper"/>
    /// </summary>
    public StamperLineFeed LineFeed { get; set; } = StamperLineFeed.Before;
    ...
    public Stamper (IStampingFileFlow stamperFileFlow, IStamperParametersProvider? stamperParametersProvider = null)
    ...
    public Stamper (IStamperParametersProvider stamperParametersProvider)
    ...
    public Stamper (string sourceFilePath, string destinationFilePath)
      : this (new StreamReader (sourceFilePath), new StreamWriter (destinationFilePath))
    {
      if (string.IsNullOrWhiteSpace (destinationFilePath)) {
        log.Error ($"Stamper: empty destinationFilePath");
    ...
    /// </summary>
    public Stamper (TextReader sourceReader, TextWriter destinationWriter)
    {
    ...
    {
      var numberOfCharacters = await m_pipeBuffer.SkipAsync (cancellationToken);
      m_currentPosition += numberOfCharacters;
    }
    ...
    {
      var numberOfCharacters = endPosition - m_currentPosition;
      m_pipeBuffer.Release (m_destinationWriter, numberOfCharacters);
      m_currentPosition = endPosition;
    }
    ...
    {
      var numberOfCharacters = endPosition - m_currentPosition;
      await m_pipeBuffer.ReleaseAsync (m_destinationWriter, numberOfCharacters, cancellationToken);
      m_currentPosition = endPosition;
    }
    ...
    public void Release ()
    {
      var writtenCharacters = m_pipeBuffer.Release (m_destinationWriter);
    ...
    {
      var writtenCharacters = await m_pipeBuffer.ReleaseAsync (m_destinationWriter);
      m_currentPosition += writtenCharacters;
    }
    ...
    public void AddLine (string line)
    {
      if (this.LineFeed.HasFlag (StamperLineFeed.Before)) {
        m_destinationWriter.WriteLine ();
      }
      m_destinationWriter.Write (line);
    ...
    {
      if (this.LineFeed.HasFlag (StamperLineFeed.Before)) {
        await m_destinationWriter.WriteLineAsync ();
      }
      await m_destinationWriter.WriteAsync (line);
      if (this.LineFeed.HasFlag (StamperLineFeed.After)) {
        await m_destinationWriter.WriteLineAsync ();
      }
    }
    ...
    public void CloseAll ()
    {
      ...
      try {
        ...
        log.Error ($"CloseAll: exception closing", ex);
        allSuccess = false;
      }
      if (allSuccess) {
        m_allClosed = true;
