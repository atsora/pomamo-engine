...
namespace Lemoine.GDBPersistentClasses.UnitTests
{
  ...
  public class AnalysisUnitTests
  {
    ...
    internal static void RunProcessingReasonSlotsAnalysis (CancellationToken cancellationToken)
    {
      var monitoredMachines = ModelDAOHelper.DAOFactory.MonitoredMachineDAO
        .FindAll ();
      foreach (var monitoredMachine in monitoredMachines) {
        RunProcessingReasonSlotsAnalysis (cancellationToken, monitoredMachine);
      }
    }

    internal static void RunProcessingReasonSlotsAnalysis (CancellationToken cancellationToken, IMonitoredMachine machine)
    {
      var analysis = new ProcessingReasonSlotsAnalysis (machine, null);
      analysis.RunOnce (cancellationToken, null);
    }
    ...
    static void ProcessModification (IModification modification)
    {
      ...
      if (modification.AnalysisStatus.Equals (AnalysisStatus.PendingSubModifications)) {
        var subModifications = modification.SubModifications.ToList ();
