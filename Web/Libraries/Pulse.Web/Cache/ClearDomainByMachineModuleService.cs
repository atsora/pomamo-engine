...
{
  /// <summary>
  /// Description of ClearDomainByMachineModuleService
  /// </summary>
  public class ClearDomainByMachineModuleService
    : GenericNoCacheService<ClearDomainByMachineModuleRequestDTO>
  {
    static readonly string BROADCAST_KEY = "Web.Broadcast";
    static readonly bool BROADCAST_DEFAULT = true;

    static readonly ILog log = LogManager.GetLogger(typeof (ClearDomainByMachineModuleService).FullName);
    ...
    /// 
    /// </summary>
    public ClearDomainByMachineModuleService ()
    {
    }
#else // !NSERVICEKIT
    /// <summary>
    /// 
    /// </summary>
    public ClearDomainByMachineModuleService (Lemoine.Core.Cache.ICacheClient cacheClient)
    {
    ...
    /// <returns></returns>
    public override async Task<object> Get (ClearDomainByMachineModuleRequestDTO request)
    {
      ...
      try {
        var task = ClearDomainAsync (m_cacheClient, request.Domain, request.MachineModuleId, request.Broadcast, rethrow: request.Wait);
        if (request.Wait) {
          await task;
        }
      }
    ...
    public static async Task ClearDomainAsync (Lemoine.Core.Cache.ICacheClient cacheClient, string domain, int machineModuleId, bool broadcast, bool rethrow)
    {
      try {
        var cleared = await cacheClient.ClearDomainByMachineModuleAsync (domain, machineModuleId);
        if (!cleared) {
          return;
        }
        ...
        if (broadcast
          ...
          using (IDAOTransaction transaction = session.BeginReadOnlyTransaction ("Web.Cache.ClearDomainByMachineModule.broadcast")) {
            webComputers = ModelDAOHelper.DAOFactory.ComputerDAO.GetWeb ();
          }
          if (1 < webComputers.Count) {
            var request = $"/Cache/ClearDomainByMachineModule/{domain}/{machineModuleId}?Broadcast=false";
