...
namespace Lemoine.Plugin.ComingEventCycleEnd
{
    ...
    , IProgressExtension
  {
    ...
    public IList<Event> GetComingEvents (IProgressResponse cycleProgressResponse)
    {
      ...
      if ((null != cycleProgressResponse) && cycleProgressResponse.EstimatedEndDateTime.HasValue) {
        var eventSeverity = new EventSeverity (EventSeverityLevel.Info);
        var cycleEndEvent = Event.CreateComingEvent (m_message, cycleProgressResponse.EstimatedEndDateTime.Value, eventSeverity, true, false);
        events.Add (cycleEndEvent);
      }
