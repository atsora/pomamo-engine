...
namespace Lemoine.Plugin.ComingEventStop
{
    ...
    , IProgressExtension
  {
    ...
    public IList<Event> GetComingEvents (IProgressResponse progressResponse)
    {
      ...
      if (progressResponse.EstimatedNextStopDateTime.HasValue) {
        var eventSeverity = new EventSeverity (EventSeverityLevel.Info);
        var nextStopEvent = Event.CreateComingEvent (m_message, progressResponse.EstimatedNextStopDateTime.Value, eventSeverity, true, false);
        var nextStopSequences = progressResponse.MachineModuleProgress.Values
          .Where (x => (null != x.NextStop) && progressResponse.EstimatedNextStopDateTime.Value.Equals (x.EstimatedNextStopDateTime))
          .Select (x => x.NextStop);
        var longText = string.Join (", ", nextStopSequences
          .Where (x => !string.IsNullOrWhiteSpace (x.Name))
          .Select (x => x.Name));
        if (!string.IsNullOrWhiteSpace (longText)) {
          nextStopEvent.LongText = longText;
        }
        events.Add (nextStopEvent);
      }
