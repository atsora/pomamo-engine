...
using Lemoine.Plugin.AutoReasonUniqueMachineModeInShift;

namespace Lemoine.AutoReason.UnitTests
{
  /// <summary>
  /// Unit tests for the class AutoReasonUniqueMachineModeInShift
  /// </summary>
  [TestFixture]
  public class AutoReasonUniqueMachineModeInShift_UnitTest : Lemoine.UnitTests.WithSecondTimeStamp
  {
    static readonly ILog log = LogManager.GetLogger (typeof (AutoReasonUniqueMachineModeInShift_UnitTest).FullName);

    /// <summary>
    /// Constructor
    /// </summary>
    public AutoReasonUniqueMachineModeInShift_UnitTest () : base (DateTime.Today.AddDays (-1))
    {
    ...
    /// </summary>
    [Test]
    public void UniqueMachineModeInShift_SimpleDetection ()
    {
          ...
          try {
            ...
            // Check that an autoreason appeared
            {
              ...
              Assert.Multiple (() => {
                ...
                Assert.That (reasonAssociation.ReasonDetails, Is.EqualTo ("Machine mode: MachineModeInactive"), "wrong reason detail");
              });
            }
          } finally {
    ...
    /// </summary>
    [Test]
    public void UniqueMachineModeInShift_DetectionWithNoFacts ()
    {
      using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        using (var transaction = session.BeginTransaction ()) {
          try {
    ...
    /// </summary>
    [Test]
    public void UniqueMachineModeInShift_DetectionWithFragments ()
    {
      using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        using (var transaction = session.BeginTransaction ()) {
          try {
    ...
    /// </summary>
    [Test]
    public void UniqueMachineModeInShift_DetectionWithFactsAcrossShifts ()
    {
      using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        using (var transaction = session.BeginTransaction ()) {
          try {
    ...
    /// </summary>
    [Test]
    public void UniqueMachineModeInShift_TwoKindsOfIdle ()
    {
      using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        using (var transaction = session.BeginTransaction ()) {
          try {
    ...
    /// </summary>
    [Test]
    public void UniqueMachineModeInShift_TwoShiftsWithoutProduction ()
    {
      using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        using (var transaction = session.BeginTransaction ()) {
          try {
    ...
    {
      var plugin = new Lemoine.Plugin.AutoReasonUniqueMachineModeInShift.Plugin ();
      plugin.Install (0);
