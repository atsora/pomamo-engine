...
using Lemoine.Plugin.AutoReasonBetweenPrograms;
...
namespace Lemoine.AutoReason.UnitTests
{
  ...
  /// </summary>
  [TestFixture]
  public class AutoReasonBetweenPrograms_UnitTest : Lemoine.UnitTests.WithSecondTimeStamp
  {
    static readonly ILog log = LogManager.GetLogger (typeof (AutoReasonBetweenPrograms_UnitTest).FullName);

    /// <summary>
    /// Constructor
    /// </summary>
    public AutoReasonBetweenPrograms_UnitTest () : base (DateTime.Today.AddHours (-12))
    {
      ...
      using (var session = ModelDAOHelper.DAOFactory.OpenSession ()) {
        using (var transaction = session.BeginTransaction ()) {
          try {
            ...
            CreateOperationSlot (machine, 6, 7, component1);
    ...
    {
      var plugin = new Lemoine.Plugin.AutoReasonBetweenPrograms.Plugin ();
      plugin.Install (0);
