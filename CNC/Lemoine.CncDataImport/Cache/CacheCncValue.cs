  ...
  internal sealed class CacheCncValue
  {
    ...
    public ICncValue GetStoredCncValue (IField field, DateTime dateTime)
    {
      ...
      if (log.IsDebugEnabled) {
        log.Debug ($"GetStoredCncValue: return {cncValue} for machineModule={m_machineModule} field={field}");
      }
                                       ...
                                       bool previousStopped)
    {
      foreach (var extension in m_endCncValueExtensions) {
        bool? result = extension.IsEndCncValueRequested (fieldCode,
                                                         ...
                                                         previousStopped);
        if (result.HasValue) {
          log.Debug ($"IsResetCncValueRequired: extension {extension} returned {result.Value}");
          return result.Value;
        }
      }

      if (previousStopped) {
        log.Debug ("IsResetCncValueRequired: previous stopped is true");
        return true;
      }
      ...
      if (newDateTime < previousDateTime) {
        log.Fatal ($"IsResetCncValueRequired: newDateTime {newDateTime} comes before previousDateTime {previousDateTime}");
      }
      else if (maxCncValueGap < newDateTime.Subtract (previousDateTime)) { // Check the gap
        if (log.IsDebugEnabled) {
          log.Debug ($"IsResetCncValueRequired: there is a gap {previousDateTime}-{newDateTime}, discontinue the CncValue for field {fieldCode}");
        }
        return true;
      }
    ...
    {
      log.Debug ($"ResetCncValue: MachineModuleId={m_machineModule.Id} fieldId={fieldId}");
      ...
      if (m_cncValues.ContainsKey (fieldId)) {
        ...
        if (0 != cncValue.Id) {
          try {
            ...
            log.Debug ($"ReloadCncValue: reloaded value is {cncValue}");
            if (null != cncValue) {
          ...
          if (m_previousValues.ContainsKey (fieldId)) {
            log.Warn ($"ReloadCncValue: reset m_previousAverageValues for machineModuleId {m_machineModule.Id} fieldId {fieldId}");
            m_previousValues.Remove (fieldId);
          }
          if (m_currentValues.ContainsKey (fieldId)) {
            log.Warn ($"ReloadCncValue: reset m_currentAverageValues for machineModuleId {m_machineModule.Id} fieldId {fieldId}");
            m_currentValues.Remove (fieldId);
          }
        }
      }
