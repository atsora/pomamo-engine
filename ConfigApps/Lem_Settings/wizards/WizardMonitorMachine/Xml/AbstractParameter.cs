...
namespace WizardMonitorMachine
{
  ...
  public abstract class AbstractParameter
  {
    ...
    /// Parameter label
    /// </summary>
    public string ParamLabel { get; private set; }
    ...
    protected AbstractParameter (XmlNode node)
    {
      ...
      if (IsValid) {
        ...
        ParamLabel = attributes["label"]?.Value ?? "";
        Description = node.InnerText;
        Help = attributes["help"]?.Value ?? "";
    ...
    /// </summary>
    /// <returns></returns>
    public bool IsParamX () => this.Name.StartsWith ("Param") && int.TryParse (this.Name.Substring ("Param".Length), out var _);
    ...
    void CreateControl ()
    {
      ...
      if (m_specializedControl is null) {
        return;
      }
#if NET6_0_OR_GREATER
      if (!string.IsNullOrEmpty (Help)) {
        if (m_specializedControl is TextBox textBox) {
      ...
      if (!string.IsNullOrEmpty (ParamLabel)) {
        m_label.Text = $"{ParamLabel} ({Description})";
      }
      else {
