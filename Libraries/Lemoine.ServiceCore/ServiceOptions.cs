...
namespace Lemoine.Service
{
  ...
  /// </summary>
  public class ServiceOptions: IMicrosoftParameters
  {
    ...
    public static ServiceOptions Parse (string[] args)
    {
      ...
      try {
        ...
        result.WithParsed<ServiceOptions> (opt => {
        ...
        if (serviceOptions is null) {
          ...
          log.Fatal ($"Parse: unexpected: serviceOptions is null");
          throw new InvalidOperationException ("Unexepcted serviceOptions value");
        }
        else {
        ...
        // For example when --version or --help is called
        log.Error ("Parse: ArgumentException raised", ex);
        throw;
      }
      catch (Exception ex) {
