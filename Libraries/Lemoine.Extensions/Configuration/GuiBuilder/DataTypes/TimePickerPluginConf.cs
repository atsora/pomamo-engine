...
{
  /// <summary>
  /// Plugin conf to consider a timePicker control to get a time
  /// </summary>
  public class TimePickerPluginConf : IPluginConfDataType
  {
    ILog log = LogManager.GetLogger (typeof (TimePickerPluginConf).FullName);
    ...
    protected void AddTimePickerControl (IPluginConfig pluginConfig, string name, string labelText)
    {
      if (this.Optional) {
        pluginConfig.AddOptionalTimePicker (name, labelText, this.WithSeconds);
      }
      else {
        pluginConfig.AddTimePicker (name, labelText, this.WithSeconds);
      }
    }
