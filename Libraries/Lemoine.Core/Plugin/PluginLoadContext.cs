    ...
    bool IsAssemblyMatch (AssemblyName x, Assembly assembly)
    {
      ...
      if (null != vx) {
        ...
        if (vx.Major != vy.Major) {
          log.Error ($"IsAssemblyMatch: major version does not match in {y.FullName}, {vx.Major} VS {vy.Major}");
          if (Lemoine.Info.ConfigSet.LoadAndGet (STRICT_VERSION_CHECK_KEY, STRICT_VERSION_CHECK_DEFAULT)) {
            return false;
          }
          else { // Not StrictVersionCheck
            return true;
          }
        }
        if (vx.Minor <= vy.Minor) {
          return true;
        }
        else {
          log.Error ($"IsAssemblyMatch: minor version does not match in {y.FullName}, {vx.Major} VS {vy.Major}");
          if (Lemoine.Info.ConfigSet.LoadAndGet (STRICT_VERSION_CHECK_KEY, STRICT_VERSION_CHECK_DEFAULT)) {
            return false;
          }
          else { // Not StrictVersionCheck
