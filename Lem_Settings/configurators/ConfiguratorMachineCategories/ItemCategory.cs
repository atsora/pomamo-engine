...
namespace ConfiguratorMachineCategories
{
  ...
  /// </summary>
  internal class ItemCategory : ItemCommon, IConfigurator
  {
    #region Constructors
    /// <summary>
    /// Description of the constructor
    /// </summary>
    public ItemCategory() : base("Categories", typeof(IMachineCategory)) {}
    ...
    protected override IList<Container.Element> GetElements()
    {
      ...
      using (IDAOSession session = ModelDAOHelper.DAOFactory.OpenSession()) {
        using (IDAOTransaction transaction = session.BeginReadOnlyTransaction()) {
          IList<IMachineCategory> elements = ModelDAOHelper.DAOFactory.MachineCategoryDAO.FindAll();
          foreach (IMachineCategory element in elements) {
    ...
    protected override void FillMachines(ListTextValue list, Container.Element element)
    {
      ...
      using (IDAOSession session = ModelDAOHelper.DAOFactory.OpenSession()) {
        using (IDAOTransaction transaction = session.BeginReadOnlyTransaction()) {
          var category = element.Item as IMachineCategory;
          if (category != null) {
            ModelDAOHelper.DAOFactory.MachineCategoryDAO.Lock(category);
            foreach (IMachine machine in category.Machines) {
      ...
      using (IDAOSession session = ModelDAOHelper.DAOFactory.OpenSession()) {
        using (IDAOTransaction transaction = session.BeginTransaction()) {
          ...
          foreach (Container.Element element in container.Elements) {
            var item = element.Item as IMachineCategory;
            if (item == null) {
              item = ModelDAOHelper.ModelFactory.CreateMachineCategory();
            }
            else {
              ModelDAOHelper.DAOFactory.MachineCategoryDAO.Lock(item);
            }

            item.Name = element.Name;
            item.Code = element.Code;
            item.DisplayPriority = displayPriority++;
            ModelDAOHelper.DAOFactory.MachineCategoryDAO.MakePersistent (item);
          }
          ...
          foreach (Container.Element element in container.DeletedElements) {
            var item = element.Item as IMachineCategory;
            ModelDAOHelper.DAOFactory.MachineCategoryDAO.Lock(item);
            ModelDAOHelper.DAOFactory.MachineCategoryDAO.MakeTransient(item);
          }
